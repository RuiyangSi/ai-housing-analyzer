...
⚠️ 如果用户询问系统功能、如何使用、有哪些页面等问题，请根据上述功能进行介绍。"""
...
📍 各省份房价数据（按均价排序，用户问房价时务必引用！）："""
                
                # 按均价降序排列（确保有价格数据）
                provinces_with_price = [p for p in provinces if p.get('avg_price', 0) > 0]
                sorted_provinces = sorted(provinces_with_price, key=lambda x: x.get('avg_price', 0), reverse=True)
                
                for i, prov in enumerate(sorted_provinces[:15], 1):  # 显示前15个
                    data_context += f"""
   {i}. {prov['name']}：均价 {prov.get('avg_price', 0):.2f}万元 | 单价 {prov.get('avg_unit_price', 0):,.0f}元/m² | 面积 {prov.get('avg_area', 0):.0f}m² | {prov['count']:,}套"""
                
                if len(sorted_provinces) > 15:
                    data_context += f"""
   ... 其他{len(sorted_provinces)-15}个省份 ..."""
                
                # 找出房价最高和最低的省份
                if sorted_provinces:
                    highest = sorted_provinces[0]
                    lowest = sorted_provinces[-1]
                    data_context += f"""

💡 房价最高：{highest['name']}（均价 {highest.get('avg_price', 0):.2f}万元，单价 {highest.get('avg_unit_price', 0):,.0f}元/m²）
💡 房价最低：{lowest['name']}（均价 {lowest.get('avg_price', 0):.2f}万元，单价 {lowest.get('avg_unit_price', 0):,.0f}元/m²）

⚠️ 重要提示：回答用户关于某地房价时，必须引用上述具体数据！"""
